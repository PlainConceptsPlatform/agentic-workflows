...
describe("workflows CLI", () => {
  ...
  it("rejects an unsupported template name", async () => {
    ...
    expect(error).toHaveBeenCalledWith("--template must be one of: agentics-app-errors|agentics-checks|agentics-error-report|agentics-maintenance|app-ci-dotnet-next|app-ci-node-monorepo|bug-report|feature-request|github-release|opencode.ci.json.");
    error.mockRestore();
  });
