...
| Merge gate skips protected-file PRs even when CI failed | `protected_changes` blocks all agent runs, not just merge. The `review_required`, `agent`, and `incomplete` jobs must allow protected-file PRs through when `needs.subject.outputs.conclusion == 'failure'` so the agent can remediate. Auto-merge is still blocked by the `conclude` job's `requires_review != 'true'` guard on the `auto-merge` disposition. |
| Reconcile cron skips a PR whose CI failed again | The reconcile script compares the latest merge-gate comment timestamp against CI completion time. A prior disposition or `incomplete` verdict newer than CI prevents re-dispatch. Remove `review` or re-dispatch manually. |
| Labelling several pull requests `merge-gate` at once runs only one | The belt is one concurrency group, and GitHub keeps a single pending run per group: each new dispatch cancels the previously pending one. `cancel-in-progress: false` protects the running gate, not the queue. The runs show as `cancelled` with no comment on the pull request. Dispatch one at a time, or let detect-pr-conflicts re-arm siblings after each merge. |
...
| Every under-limit issue skips the agent in 0s, every gate green, incomplete re-dispatches until the issue parks | A custom job that skips in the happy path (an exclusive alternative: refusal on `too_big != 'true'`) sits in the agent's `needs`, which gh-aw assembles automatically. With no status function in the top-level `if:` GitHub prepends `success()`, and a skipped need fails it. Add `&& !failure()` to the gate, exclude the false branch from `incomplete`'s `if:`, and have the false branch's job release the reservation itself (Pliny-Bot #320-322) |
| A feature runs "green" on every merge but never once executed (visual-verify: zero screenshots ever) | A step did `uses: ./.github/workflows/<worker>.lock.yml`. A step cannot call a reusable workflow: the runner resolves the path as a local action and dies with "Can't find action.yml", swallowed by `continue-on-error`. Call workers from a job-level `uses:` (router pattern) — and if the call must sit downstream of the worker's own agent, the compiler's needs-hoisting forces a cycle, so dispatch the router operation instead (the dispatch-triage pattern) |
